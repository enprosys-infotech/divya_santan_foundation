...
export default function JoinPage() {
  ...
  useEffect(() => {
    const meta = getDictionary().join.meta;
    ...
    <>
      ...
      {activeTab === "events" && (
        <>
                ...
                <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mt-3 text-muted-foreground">
                      Join us for workshops, seminars, and community gatherings focused on Garbh
                      Sanskar education and awareness. Check back soon for upcoming events.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Section className="bg-warm">
            <SectionHeading eyebrow={eventsCopy.past.eyebrow} title={eventsCopy.past.title} />
          ...
          <Section>
            ...
            <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {TESTIMONIALS.map((testimonial) => (
                <TestimonialCard key={testimonial.id} {...t.content.testimonials[testimonial.id]} />
              ))}
            </div>
          </Section>
              ...
              <div className="mt-14 grid gap-8 sm:grid-cols-3">
                <div className="surface-card p-6">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    {testimonialsCopy.impact.families}
                  </p>
                </div>
                <div className="surface-card p-6">
                  <p className="text-4xl font-bold text-primary">500+</p>
                  <p className="mt-2 text-sm text-muted-foreground">
                    {testimonialsCopy.impact.births}
                  </p>
                </div>
                <div className="surface-card p-6">
                  <p className="text-4xl font-bold text-primary">95%</p>
                  <p className="mt-2 text-sm text-muted-foreground">
                    {testimonialsCopy.impact.satisfaction}
                  </p>
                </div>
              </div>
            </div>
