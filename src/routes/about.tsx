...
import { Flower2, Leaf, HeartHandshake, CheckCircle2, Globe2, GraduationCap, Building2, Stethoscope, Users } from "lucide-react";
        ...
        <Section className="relative z-10 pt-12">
          {activeTab === "vision" && (
            <div className="mx-auto max-w-4xl space-y-16 animate-rise">
              ...
              <div className="space-y-6 text-center">
                <p className="text-sm font-medium uppercase tracking-[0.2em] text-primary">{aboutData.vision.intro}</p>
                <h2 className="text-3xl leading-tight text-ink sm:text-4xl">{aboutData.vision.title}</h2>
              ...
              <div className="surface-card bg-warm p-8 sm:p-12 text-center border-gold/30 relative overflow-hidden">
                ...
                <h3 className="text-2xl font-medium text-ink relative z-10">{aboutData.vision.coreObjectiveTitle}</h3>
                <div className="mt-6 flex flex-wrap justify-center gap-3 relative z-10">
                  {aboutData.vision.coreObjectiveList.map((obj, i) => (
                    <span key={i} className="rounded-full bg-background/80 border border-border px-4 py-2 text-sm font-medium text-ink shadow-sm">
                      {obj}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-8">
                <div className="text-center">
                  ...
                  <p className="mt-4 text-muted-foreground leading-relaxed">{aboutData.section8.description}</p>
                  <p className="mt-4 text-muted-foreground leading-relaxed font-medium text-ink">{aboutData.section8.vision}</p>
                </div>

                <div className="grid gap-8 sm:grid-cols-2">
                  <div className="surface-glass p-6 rounded-2xl border-primary/20">
                    <h4 className="font-medium text-lg text-ink mb-4">{aboutData.section8.whatIsSection8.title}</h4>
                  ...
                  <div className="surface-glass p-6 rounded-2xl border-secondary/20">
                    <h4 className="font-medium text-lg text-ink mb-4">{aboutData.section8.largerPurpose.title}</h4>
                    <div className="space-y-3 text-sm text-muted-foreground">
                ...
                <div className="mt-10">
                  <h4 className="font-medium text-lg text-ink mb-6 text-center">{aboutData.labels.keyObjectivesTitle}</h4>
          ...
          {activeTab === "pillars" && (
            <div className="mx-auto max-w-5xl space-y-12 animate-rise">
              <div className="text-center mb-10">
                <p className="text-[0.68rem] uppercase tracking-[0.2em] text-secondary">{aboutData.labels.pillarsEyebrow}</p>
                <h2 className="mt-3 text-3xl leading-tight text-ink sm:text-4xl">{aboutData.labels.pillarsTitle}</h2>
              ...
              <div className="grid gap-8 lg:grid-cols-2">
                {aboutData.fivePillars.map((pillar) => {
                  ...
                  return (
                    <div key={pillar.id} className="surface-card p-6 sm:p-8 flex flex-col gap-6 relative overflow-hidden group">
                      ...
                      <div className="space-y-6 relative z-10">
                        {pillar.sections.map((section, idx) => (
                          <div key={idx}>
                            <h4 className="font-medium text-sm text-primary mb-2">{section.subtitle}</h4>
                            {section.body && <p className="text-sm text-muted-foreground mb-3 leading-relaxed">{section.body}</p>}
                            <ul className="space-y-2">
                              {section.points.map((point, pIdx) => (
                                <li key={pIdx} className="flex items-start gap-2 text-sm text-ink leading-relaxed">
          ...
          {activeTab === "journey" && (
            <div className="mx-auto max-w-3xl space-y-12 animate-rise">
              <div className="text-center mb-10">
                <p className="text-[0.68rem] uppercase tracking-[0.2em] text-gold-foreground">{aboutData.labels.historyEyebrow}</p>
                <h2 className="mt-3 text-3xl leading-tight text-ink sm:text-4xl">{aboutData.labels.historyTitle}</h2>
                <span className="mx-auto mt-4 block h-px w-16 bg-gold/70" />
              </div>
                  ...
                  {aboutData.leadership.patrons.map(({ name, role, img, contributions }) => (
                    <article key={name} className="surface-card surface-card-hover flex gap-4 p-5">
                      <div className="relative shrink-0">
                        <span aria-hidden className="pointer-events-none absolute -inset-1 rounded-full bg-gold/12 blur-md" />
                        <img
                          src={img}
                      ...
                      <div className="min-w-0 flex flex-col justify-center">
                        ...
                        <p className="mt-0.5 text-[0.62rem] uppercase tracking-[0.18em] text-gold-foreground">{role}</p>
                      </div>
                    </article>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="mt-10 grid gap-5 lg:grid-cols-2">
                  {aboutData.leadership.founders.map(({ name, role, credentials, img, desc }) => (
                    <article key={name} className="surface-card surface-card-hover flex flex-col sm:flex-row overflow-hidden p-0 h-full">
                ...
                <div>
                  ...
                  <div className="mt-8 flex flex-col gap-4">
                    {aboutData.leadership.advisors.map(({ name, role, img }) => (
                      <article key={name} className="surface-card surface-card-hover flex gap-4 border-l-2 border-l-primary/40 p-5">
                        <img
                          src={img}
                          alt={`Portrait of ${name}`}
                        ...
                        <div className="min-w-0 flex flex-col justify-center">
                          ...
                          <p className="text-[0.62rem] uppercase tracking-[0.18em] text-primary">{role}</p>
                        </div>
                      </article>
                    ))}
                  </div>
                  ...
                  <div className="mt-8 surface-card p-6 bg-warm">
                    <div className="flex flex-wrap gap-2">
                      {aboutData.leadership.executiveCouncil.map((name, i) => (
                        <span key={i} className="inline-flex items-center rounded-full border border-border bg-background px-3 py-1.5 text-xs text-ink shadow-sm hover:border-secondary/30 transition-colors">
                          {name}
                        </span>
                      ))}
                    </div>
