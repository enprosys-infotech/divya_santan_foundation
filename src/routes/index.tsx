...
import { FeatureCard, KnowledgeCard, CourseCard, TestimonialCard, MobileAppCTA } from "@/components/site/Cards";
    ...
    <>
              ...
              {AUDIENCE_PILLS.map((pill) => {
                ...
                return (
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      document.getElementById("who-can-benefit")?.scrollIntoView({ behavior: "smooth", block: "start" });
                    }}
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-ink leading-tight">{copy.hero.pills[pill.id].label}</span>
        ...
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="mt-8 grid gap-12 lg:grid-cols-[1fr_1px_1fr] lg:gap-0">
            ...
            <div className="lg:pl-12">
              ...
              >
                {gsTab === "etymology" && (
                  <p>{copy.foundations.whatIs.philosophy}</p>
                )}
                {gsTab === "practice" && (
                  <p>{copy.foundations.whatIs.approach}</p>
                )}
                {gsTab === "science" && (
                  <p>{copy.foundations.whatIs.science}</p>
                )}
              </div>
          ...
          <details className="group mt-12 border-t border-border pt-5">
            <summary className="flex cursor-pointer list-none items-center gap-2 text-xs text-muted-foreground hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">
              <ChevronDown className="h-3.5 w-3.5 shrink-0 transition-transform duration-200 group-open:rotate-180" aria-hidden="true" />
              <span className="uppercase tracking-[0.18em]">⚕️ Medical disclaimer & source</span>
            </summary>
            <div className="mt-3 space-y-1.5 pl-5">
              <p className="text-xs leading-relaxed text-muted-foreground">{copy.foundations.whatIs.disclaimer}</p>
              <p className="text-[0.68rem] italic text-muted-foreground/70">{copy.foundations.whatIs.source}</p>
            </div>
          </details>
      ...
      <section className="border-b border-border bg-accent/50 px-5 py-10 sm:px-8">
        <div className="mx-auto w-full max-w-6xl">
          <div className="mb-8 text-center">
            <p className="text-[0.68rem] uppercase tracking-[0.3em] text-secondary">{copy.foundations.whatIs.whyMatters.title}</p>
            <p className="mx-auto mt-4 max-w-3xl text-base leading-relaxed text-muted-foreground">{copy.foundations.whatIs.whyMatters.body}</p>
          </div>
      ...
      >
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="mt-4 text-center">
            ...
            <p className="mt-2 text-sm italic text-ink/60">— {copy.socialMission.vasudhaivaMeaning}</p>
          </div>
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-2">
            ...
            <p className="text-base leading-relaxed text-ink/80">{copy.socialMission.epigenetics}</p>
          </div>
      ...
      <section className="border-b border-border bg-background px-5 py-12 sm:px-8 sm:py-16">
        <div className="mx-auto w-full max-w-6xl">
          <div className="text-center">
            <p className="text-[0.68rem] uppercase tracking-[0.3em] text-secondary">{copy.leadership.eyebrow}</p>
            <h2 className="mt-3 text-2xl text-ink sm:text-3xl">{copy.leadership.title}</h2>
            <p className="mx-auto mt-3 max-w-2xl text-base text-muted-foreground">{copy.leadership.subtitle}</p>
          </div>
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <div className="surface-card flex flex-col items-center p-6 text-center">
              <div className="h-24 w-24 overflow-hidden rounded-full border-2 border-gold/30 shadow-md">
                <img src="/Avdheshanand-maharaj.png" alt={copy.leadership.members.avdheshanand.name} className="h-full w-full object-cover" loading="lazy" />
              </div>
              <h3 className="mt-4 text-base font-medium text-ink">{copy.leadership.members.avdheshanand.name}</h3>
              <p className="mt-1 text-xs uppercase tracking-[0.2em] text-gold-foreground">{copy.leadership.members.avdheshanand.role}</p>
              <p className="mt-3 text-sm text-muted-foreground">{copy.leadership.members.avdheshanand.bio}</p>
            </div>
            <div className="surface-card flex flex-col items-center p-6 text-center">
              <div className="h-24 w-24 overflow-hidden rounded-full border-2 border-primary/30 shadow-md">
                <img src="/anil-sir.png" alt={copy.leadership.members.anil.name} className="h-full w-full object-cover" loading="lazy" />
              </div>
              <h3 className="mt-4 text-base font-medium text-ink">{copy.leadership.members.anil.name}</h3>
              <p className="mt-1 text-xs uppercase tracking-[0.2em] text-secondary">{copy.leadership.members.anil.role}</p>
              <p className="mt-3 text-sm text-muted-foreground">{copy.leadership.members.anil.bio}</p>
            </div>
            <div className="surface-card flex flex-col items-center p-6 text-center">
              <div className="h-24 w-24 overflow-hidden rounded-full border-2 border-primary/30 shadow-md">
                <img src="/vinod-sir.png" alt={copy.leadership.members.vinod.name} className="h-full w-full object-cover" loading="lazy" />
              </div>
              <h3 className="mt-4 text-base font-medium text-ink">{copy.leadership.members.vinod.name}</h3>
              <p className="mt-1 text-xs uppercase tracking-[0.2em] text-secondary">{copy.leadership.members.vinod.role}</p>
              <p className="mt-3 text-sm text-muted-foreground">{copy.leadership.members.vinod.bio}</p>
            </div>
          </div>
          <div className="mt-8 text-center">
            <Button asChild variant="outline" size="sm">
              <Link to="/about" className="gap-1.5">{copy.leadership.meetTeam} <ArrowRight className="h-3.5 w-3.5" /></Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <Section className="border-b border-border">
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {GARBH_BASICS.map((basic) => {
            const iconMap: Record<string, any> = { preConception: "leaf", pregnancyJourney: "lotus", garbhSamvad: "om", familyEnvironment: "sun" };
            return (
              <FeatureCard key={basic.id} spiritualIcon={iconMap[basic.id] as any} iconVariant={basic.iconVariant} {...t.content.basics[basic.id]} />
            );
          })}
        </div>
        <div className="gold-rule mx-auto mt-16 mb-6 max-w-xs" />
        <p className="mx-auto mb-6 max-w-xs text-center text-[0.68rem] uppercase tracking-[0.26em] text-gold-foreground">{copy.foundations.pillarsLabel}</p>
      ...
      <Section id="hub">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-2">
          {(["freeServices", "videos", "courses", "knowledge", "consultation"] as HubTab[]).map((tab) => {
