      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-xl text-ink sm:text-2xl">
                {format(copy.carouselTitle, { total })}
              </h3>
              <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-0.5 text-[0.68rem] font-semibold text-primary">
                <Touchpad className="h-3 w-3" /> {copy.slingBadge}
              </span>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              {format(copy.carouselHint, { total })}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="h-11 w-11 rounded-full border-primary/30 transition-transform active:scale-90"
              >
        ...
        <div className="mt-6 overflow-hidden" ref={emblaRef}>
          <div className="flex touch-pan-y gap-5 py-2">
            {SYNERGY_PILLARS.map((pillar, index) => {
              const item = pillarCopy(pillar.id);
              return (
                <div
                  key={pillar.id}
                ...
                >
                  <button
                    ...
                    onClick={() => selectPillar(index)}
                    className={cn(
                      "surface-card surface-card-hover block h-full w-full cursor-pointer p-6 text-left transition-all duration-300 active:scale-[0.98]",
                      selectedIndex === index
                        ? "border-primary/40 bg-gradient-to-b from-background via-primary/5 to-accent/30 shadow-md ring-2 ring-primary/60"
                        : "bg-background",
                    )}
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-3xl leading-none">{pillar.symbol}</span>
                      <span className="rounded-full border border-border bg-accent px-2.5 py-0.5 text-[0.65rem] font-medium uppercase tracking-[0.16em] text-muted-foreground">
                        {item.category}
                      </span>
                    </div>

                    <h4 className="mt-4 text-base font-medium text-ink">{item.ancientTitle}</h4>
                    <p className="mt-1 text-xs font-semibold text-secondary">{item.scienceTitle}</p>
                    ...
                    <div className="mt-5 flex items-center justify-between border-t border-border/60 pt-3 text-[0.7rem]">
                      <span className="text-muted-foreground">{copy.validationLabel}</span>
        ...
        <div className="mt-6 flex justify-center gap-2">
          {SYNERGY_PILLARS.map((pillar, index) => (
            <button
              ...
              className="flex min-h-[20px] min-w-[20px] cursor-pointer items-center justify-center rounded-full transition-all duration-300"
            >
              <span
                className={cn(
                  "block h-2.5 rounded-full transition-all duration-300",
                  selectedIndex === index ? "w-7 bg-secondary" : "w-2.5 bg-border hover:bg-secondary/50",
                )}
              />
            </button>
          ))}
